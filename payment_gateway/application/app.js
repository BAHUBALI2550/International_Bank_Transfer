    ...
    try {
        ...
        if( customerPassword.length >= 8 && hasNumber && hasChar) {
            ...
            if(loginResult.success) {
                console.log("customer is successfully created");
...
app.post("/createAccount", async (req, res) => {
    const {customerID, bankID, balance, currency} = req.body;
    try {
        const result = await createAccount(customerID, bankID, balance, currency);
        if (!result.success) {
    ...
    try {
        ...
        if(loginResult.success)
        {
            const result = await createPayment(
              ...
              req.session.customerID = customerID;
              if(result.success) {
                const message = "Money Successfully Transfered.";
...
app.post("/deleteAccount", async (req, res) => {
    ...
    try {
        if(confirmation !== "YES") {
            const errorMessage = "Enter correct confimation word to delete the account.";
...
app.post("/updateBalance", async (req, res) => {
  ...
  app.post("/createBank", async (req, res) => {
    ...
    try {
      ...
      if (isSuccess) {
        console.log("Bank creation is complete.");
  ...
  app.get("/k6-test", (req, res) => {
    res.send("K6 test completed successfully");
  });
